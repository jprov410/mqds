...
! using SQC with a harmonic bath model in the diabatic basis
...
SUBROUTINE calculate_sqc_absorption_mpi
  ...
  COMPLEX(dp) :: sum_resp_func(0 : nbstep / dump)
  ...
  weight = 0.0_dp ; sum_resp_func = ( 0.0_dp, 0.0_dp )
          ...
          IF ( dipcom_gstate(istate, istatet) /= 0.0_dp ) THEN
              ...
              DO itraj=1, INT(ntraj/npes)
                  ...
                  redmat(:, :, itime) = redmat(:, :, itime) &
                          + weight * sqc_redmat(x_map, p_map, itime)
  ...
  ELSE
      ...
      END DO
      sum_resp_func = sum_resp_func / DBLE(npes)
      CALL write_linear_response(method, sum_resp_func, printstep)
  END IF

  ! Deallocate spectroscopy arrays
  CALL finalize_spectroscopy

END SUBROUTINE calculate_sqc_absorption_mpi
